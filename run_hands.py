...
if __name__ == "__main__":
    ...
    TC_rolling_strategy=[{"max":4,"weight":{1:1,2:1,3:1,4:1},"remaining_decks":5}, #0
                         ...
                         {"max":4,"weight":{1:1.5,2:1,3:0.66,4:0.5},"remaining_decks":5}, #6
    ...
    for r in range(ROUNDS):
        ...
        round = Round(roundbet(math.floor(2*sh._trueCount)/2), sh, basic_strategy[str(strategy_code)],player_number=player_number,min_bet=min_bet)  
        round.play_round()
    ...
    for i in range(len(TC_list)):
        if not (math.floor(TC_list[i]*2)/2 in EV_distribution_ag_TC):
            EV_distribution_ag_TC[math.floor(TC_list[i]*2)/2] = {"frequency":1, "accumulated_EV":pnl_per_original_bet_list[i], "average EV":0}
        else:
            EV_distribution_ag_TC[math.floor(TC_list[i]*2)/2]["frequency"] += 1
            EV_distribution_ag_TC[math.floor(TC_list[i]*2)/2]["accumulated_EV"] += pnl_per_original_bet_list[i]
